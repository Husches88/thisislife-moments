...
/*
  Deine Instagram-Seite:
  Hier später einfach die echte Instagram-URL eintragen.
*/
...
async function load() {
  ...
  if (!response.ok) throw new Error("images.json konnte nicht geladen werden");

  const data = await response.json();
  items = Array.isArray(data) ? data : (data.images || []);

  const heroItem = items.find(x => x.hero);
  if (heroItem) {
    hero.style.backgroundImage = `url("${imageUrl(heroItem.file)}")`;
  }

  const galleryItems = items.filter(x => !x.hero);
  statusEl.textContent = `${galleryItems.length} Moments`;

  if (!galleryItems.length) {
    gallery.innerHTML = '<div class="empty"><strong>Noch keine Fotos.</strong><br>Lege Bilder in den Ordner <code>images</code> und trage sie in <code>images.json</code> ein.</div>';
    return;
  }
...
function openLightbox(i) {
  ...
  current = i;
  boxImg.src = imageUrl(galleryItems[i].file);
  boxImg.alt = galleryItems[i].title || "";
...
function move(delta) {
  ...
  boxImg.src = imageUrl(galleryItems[current].file);
  boxImg.alt = galleryItems[current].title || "";
}
...
load().catch(error => {
  ...
  gallery.innerHTML = `
    <div class="empty">
      <strong>Galerie konnte nicht geladen werden.</strong><br>
      Prüfe, ob <code>images.json</code> im Hauptordner liegt und gültiges JSON enthält.
    </div>`;
});
